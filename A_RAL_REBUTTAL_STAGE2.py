...
import clip
from A_RAL_STAGE2_dataloader import LMDBDataset as LMDBdst_jpeg
from A_RAL_STAGE2_dataloader import DataPrefetcher as DataPrefetcher_jpeg
        ...
        with tqdm(total=train_dataset_len, desc=f"Train Epoch {epoch+1}", leave=False,disable= not acc.is_main_process) as pbar:
            ...
            while batch is not None:
                ...
                with acc.accumulate(model):
                    ...
                    optimizer.zero_grad()
                    if cfg['use_2d_traj']:
                        rgb_camera_left, rgb_camera_wrist, rgb_camera_top, new_action_2d_top, new_action_2d_top_rest = preprocessor.rgb_process(batch['rgb_camera_left'], batch['rgb_camera_wrist'], batch['rgb_camera_top'], batch['actions_2d'],batch['traj_2d_preds'],train=True)
                    else:
                        rgb_camera_left, rgb_camera_wrist, rgb_camera_top = preprocessor.rgb_process(batch['rgb_camera_left'], batch['rgb_camera_wrist'], batch['rgb_camera_top'],train=True)
                    ...
                    # cv2.imwrite("visualization/tmp.png", rgb_vis)  
                    obs_mask = batch['mask'][..., 0]
                    pred = model(
                        rgb_left=rgb_camera_left,
                        rgb_wrist=rgb_camera_wrist,
                    ...
                    loss['rgb_camera_wrist'] = masked_loss(pred['obs_wrist_preds'], pred['obs_wrist_targets'], obs_mask, cfg['skip_frame'], F.mse_loss)
                    ...
                    loss['total_loss'] = (loss['traj_2d_preds'] if cfg['use_2d_traj'] else 0) \
                                   + loss['rgb_camera_left'] + loss['rgb_camera_wrist'] + loss['rgb_camera_top'] \
                ...
                if batch_idx % eval_steps == 0:
                    with torch.no_grad():
                        ...
                        if cfg['use_2d_traj']:
                            rgb_camera_left, rgb_camera_wrist, rgb_camera_top, new_action_2d_top, new_action_2d_top_rest = preprocessor.rgb_process(batch['rgb_camera_left'], batch['rgb_camera_wrist'], batch['rgb_camera_top'], batch['actions_2d'],batch['traj_2d_preds'],train=False)
                        else:
                            rgb_camera_left, rgb_camera_wrist, rgb_camera_top = preprocessor.rgb_process(batch['rgb_camera_left'], batch['rgb_camera_wrist'], batch['rgb_camera_top'],train=False)
                            new_action_2d_top = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='A_RAL_Real_Robot_configs_debug.json', help='Path to the config JSON file')
