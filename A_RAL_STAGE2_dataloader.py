...
class LMDBDataset(Dataset):
    def __init__(self, lmdb_dir, sequence_length, chunk_size, action_mode, action_dim, start_ratio, end_ratio):
        ...
        self.dummy_rgb_camera_wrist = torch.zeros(sequence_length, 3, ORIGINAL_STATIC_RES_H, ORIGINAL_STATIC_RES_W, dtype=torch.uint8)
        ...
        self.dummy_rgb_camera_wrist_padding = torch.zeros(sequence_length, 3, ORIGINAL_STATIC_RES_W, ORIGINAL_STATIC_RES_W, dtype=torch.uint8)
    ...
    def __getitem__(self, idx):
        ...
        rgb_camera_wrist = self.dummy_rgb_camera_wrist_padding.clone()
        ...
        for i in range(self.sequence_length):
            if loads(self.txn.get(f'cur_episode_{idx+i}'.encode())) == cur_episode:
                ...
                get_rgb_camera_wrist = decode_jpeg(loads(self.txn.get(f'rgb_camera_wrist_{idx+i}'.encode())))
                rgb_camera_wrist[i] = F.pad(
                                        get_rgb_camera_wrist,
                                        pad=(0, 0, 80, 80),  # 在高度方向填充
                                        mode='replicate'  # 使用边缘像素填充
                                    )
                # visualization
                ...
                # rgb_camera_wrist_padding = rgb_camera_wrist[i].permute(1, 2, 0).cpu().numpy()
                ...
                # cv2.imwrite("visualization/rgb_camera_wrist_padding.png",rgb_camera_wrist_padding)
                # end
                ...
                # traj_2d data
                traj_2d_preds_len =len(loads(self.txn.get(f'traj_2d_left_{idx+i}'.encode())))
                traj_2d_preds[i,:min(100,traj_2d_preds_len)] = loads(self.txn.get(f'traj_2d_left_{idx+i}'.encode()))[:min(100,traj_2d_preds_len)]
                if traj_2d_preds_len < 100:
                    traj_2d_preds[i,traj_2d_preds_len:] = traj_2d_preds[i,traj_2d_preds_len-1] # 补齐长度
                traj_2d_preds[i] = traj_2d_preds[i]  + torch.tensor([0, 80]) # 坐标转换
                future_2d_actions = loads(self.txn.get(f'traj_2d_left_init_{cur_episode}'.encode())) # 只用当前episode的2d action 为了和inference保持一致 且快速验证
                future_2d_actions = future_2d_actions + torch.tensor([0, 80]) # 坐标转换
                actions_2d[i] = resample_sequence_adapter(future_2d_actions, 30)
                # visualization
                # image =  rgb_camera_left[i] 
                # action = actions_2d[i] 
                ...
                # cv2.imwrite("visualization/rgb_camera_left_with_2d_action.png", rgb_vis)  
                # print('fsc_test')
        ...
        return {
            ...
            'rgb_camera_wrist': rgb_camera_wrist,
            # 'inst': inst,
...
if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
     # Preparation
    cfg = json.load(open('A_RAL_Real_Robot_configs_debug.json'))
    # 获得stats
