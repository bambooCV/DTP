...
class Real_Robot_2D_PreProcess(): 
    ...
    def rgb_process(self, rgb_camera_top, action_2d,train=False):

        rgb_camera_top = rgb_camera_top.float()*(1/255.)
        if train:
            rgb_camera_top,shift = RandomShiftsAug(rgb_camera_top, self.rgb_static_pad)
            new_action_2d = shifts_2d_action_to_Aug(action_2d, shift,self.rgb_static_pad,rgb_camera_top.shape[-1],rgb_camera_top.shape[-2])
            # 保护防止2d点越界
            new_action_2d = torch.clamp(new_action_2d, 0, 640)
        else:
            new_action_2d = action_2d.clone()
        rgb_camera_top = self.resize(rgb_camera_top)
        new_action_2d = resize_points(new_action_2d, (640,640), (224,224))
        rgb_camera_top = (rgb_camera_top - self.rgb_mean) / (self.rgb_std + 1e-6)
        return rgb_camera_top, new_action_2d
