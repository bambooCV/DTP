...
class GR1(nn.Module):
    def __init__(
        ...
        self.embed_img_right = torch.nn.Linear(self.img_feat_dim, hidden_size)
        ...
        self.embed_patch_right = torch.nn.Linear(self.patch_feat_dim, hidden_size) 
        ...
        self.obs_right_queries = nn.Embedding(self.n_patch_latents + 1, self.hidden_size)
    ...
    # real robot tranform rgb, hand_rgb, to rgb_left,rgb_right, rgb_top
    def forward(self, 
                rgb_left, 
                rgb_right,
                rgb_top, 
        ...
        if self.use_multi_rgb:
            obs_right_embeddings, obs_right_patch_embeddings = self.model_mae(
                rgb_right.view(batch_size*sequence_length, c, h, w))
            obs_right_embeddings = obs_right_embeddings.view(batch_size, sequence_length, -1)  # (b, t, img_feat_dim)
        ...
        if self.fwd_pred:
            ...
            if self.fwd_pred_multiview:
                rgb_right = rgb_right.reshape(shape=(batch_size, sequence_length, 3, h_p, p, w_p, p))
                obs_right_targets = rgb_right.permute(0, 1, 3, 5, 4, 6, 2)
                obs_right_targets = obs_right_targets.reshape(shape=(batch_size, sequence_length, h_p * w_p, (p**2)*3))  # (b, t, n_patches, p*p*3)
                if not self.without_norm_pixel_loss:
                    # norm the target 
                    obs_right_targets = (obs_right_targets - obs_right_targets.mean(dim=-1, keepdim=True)
                        ) / (obs_right_targets.var(dim=-1, unbiased=True, keepdim=True).sqrt() + 1e-6)   
        ...
        if self.use_multi_rgb:
            obs_right_patch_embeddings = obs_right_patch_embeddings.unsqueeze(1)
            obs_right_patch_embeddings = self.perceiver_resampler(obs_right_patch_embeddings)
            obs_right_patch_embeddings = obs_right_patch_embeddings.squeeze(1)
            obs_right_patch_embeddings = obs_right_patch_embeddings.view(batch_size, sequence_length, self.n_patch_latents, self.patch_feat_dim)  # (b, t, n_patch_latents, patch_feat_dim)
        ...
        if self.use_multi_rgb:
            obs_right_embeddings = self.embed_img_right(obs_right_embeddings.float())  # (b, t, h)
            obs_right_patch_embeddings = self.embed_patch_right(obs_right_patch_embeddings.float())  # (b, t, n_patch_latents, h)
        ...
        if self.use_multi_rgb:
            obs_right_embeddings = obs_right_embeddings + time_embeddings
            obs_right_patch_embeddings = obs_right_patch_embeddings + time_embeddings.view(sequence_length, 1, self.hidden_size)
        ...
        if self.use_2d_traj:
            ...
            stacked_inputs = torch.cat(
                    ...
                    obs_embeddings), dim=2)  # (b, t, n_tokens, h)
        if self.use_multi_rgb:
            obs_right_embeddings = obs_right_embeddings.view(batch_size, sequence_length, 1, self.hidden_size)
            stacked_inputs = torch.cat(
                (stacked_inputs,
                 obs_right_patch_embeddings, 
                 obs_right_embeddings), dim=2)  # (b, t, n_tokens, h)
        ...
        if self.fwd_pred:
            ...
            if self.fwd_pred_multiview:
                obs_right_queries = self.obs_right_queries.weight # 10, h
                obs_right_queries = obs_right_queries.view(1, 1, self.n_patch_latents+1, self.hidden_size).repeat(batch_size, sequence_length, 1, 1)
                stacked_inputs = torch.cat((stacked_inputs, obs_right_queries), dim=2)
        ...
        n_obs_tokens = 1
        n_right_patch_tokens = self.n_patch_latents
        n_right_obs_tokens = 1
        ...
        if self.use_multi_rgb:
            n_tokens += n_right_obs_tokens
            n_tokens += n_right_patch_tokens
        ...
        if self.fwd_pred:
            ...
            if self.fwd_pred_multiview:
                obs_right_query_token_start_i = n_tokens
                n_tokens += (n_patch_tokens + n_obs_tokens) 
        ...
        if self.use_multi_rgb:
            unmasked_tokens += n_right_obs_tokens
            unmasked_tokens += n_right_patch_tokens
        ...
        if self.fwd_pred:
            ...
            if self.fwd_pred_multiview:
                obs_right_query_attention_mask = torch.zeros((batch_size, sequence_length, n_patch_tokens + n_obs_tokens), dtype=torch.long, device=stacked_inputs.device)
                stacked_attention_mask = torch.cat((stacked_attention_mask, obs_right_query_attention_mask), dim=2)
        ...
        if self.fwd_pred:
            ...
            if self.fwd_pred_multiview:
                obs_pred_right = self.decoder_embed(x[:, :, obs_right_query_token_start_i:(obs_right_query_token_start_i + self.n_patch_latents + n_obs_tokens)])
                obs_pred_right_ = torch.cat([obs_pred_right, mask_tokens], dim=2)
                obs_pred_right_ = obs_pred_right_.reshape(-1, obs_pred_right_.shape[-2], obs_pred_right_.shape[-1])
                for blk in self.decoder_blocks:
                    obs_pred_right_ = blk(obs_pred_right_)
                obs_pred_right_ = self.decoder_norm(obs_pred_right_)
                obs_right_preds = self.decoder_pred(obs_pred_right_)
                obs_right_preds = obs_right_preds.reshape(batch_size, sequence_length, -1, obs_right_preds.shape[-1])
                obs_right_preds = obs_right_preds[:, :, (self.n_patch_latents+n_obs_tokens):]
        ...
        prediction = {
            ...
            'obs_right_preds': obs_right_preds,
            'obs_right_targets': obs_right_targets,
