...
from traj_predict.Real_Robot_traj_diffusion_data_loader import resample_sequence_adapter
preparing = True
...
class TrajPredictPolicy(nn.Module):
    def __init__(
        ...
        checkpoint_vit = torch.load("/home/ps/Dev/bamboo/Pretrain_Model/vit/mae_pretrain_vit_base.pth")
        model_mae.load_state_dict(checkpoint_vit['model'], strict=False)
        # language encoders model
        model_clip, _ = clip.load("/home/ps/Dev/bamboo/Pretrain_Model/clip/ViT-B-32.pt",device="cpu") 
...
class DTP_Evaluation():
    def __init__(self,cfg):
        ...
        if self.use_2d_traj:
            ...
            traj_model_path = "/home/ps/Dev/bamboo/Pretrain_Model/ral_rebuttal/RAL_Rebuttal_stage1_0427.pth"
    ...
    def infer(self,obs,task_name='open the upper drawer',key=None):
        # Language
        ...
        if self.use_2d_traj:
            ...
            if self.rollout_step_counter == 0 or self.diff_flag == True:
                ...
                with torch.no_grad():
                    ...
                    # re_out_action = resample_sequence_adapter(re_out_action.squeeze(0), 30, noise_flag=False).unsqueeze(0)
                    self.traj_2d_list = []
                    for _ in range(10):
        ...
        if self.use_2d_traj:
            rgb_vis = rgb_left[:,80:-80,:].permute(1, 2, 0).numpy().copy()
            point_2d_pred = prediction['traj_2d_preds'][0][-1] * 640 - torch.tensor([0, 80]).to(self.device)
            re_out_action_ori = resize_points(re_out_action.clone(), (224,224), (640,640))
            re_out_action_ori = re_out_action_ori.squeeze(0).squeeze(0)
            for index, point_2d in enumerate(re_out_action_ori - torch.tensor([0, 80]).to(self.device)):
                color = (
                    int(255 * (index / 30)),  # 红色分量
                    int(206 * (index / 30)),  # 绿色分量
                    int(135 * (index / 30))   # 蓝色分量
...
def main_DTP():
    ...
    fake_obs = {
        'images': {
            ...
            'front':fake_top_image,
        },

        'arm_joints': {
            'single': np.array([-0.04249195754528046, 0.06590165197849274, -0.016230255365371704, -1.6042454242706299, -0.09310498088598251, 1.5987823009490967, 0.02900964766740799, 0.0]),
        }
