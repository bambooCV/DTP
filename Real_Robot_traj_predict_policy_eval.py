                    ...
                    if eval_flag:
                        ...
                        for batch_idx in range(image.shape[0]):
                            for seq_idx in range(image.shape[1]):
                                rgb_camera_top = image[batch_idx][seq_idx][:, 80:-80, :].permute(1, 2, 0).cpu().numpy().copy()
                                ...
                                #     )
                                #     cv2.circle(rgb_camera_top, tuple(point_2d.int().tolist()), radius=8, color=(255, 255, 255), thickness=-1)
                                #     cv2.circle(rgb_camera_top, tuple(point_2d.int().tolist()), radius=6, color=color, thickness=-1)
                                # cv2.putText(rgb_camera_top, batch["inst"][batch_idx], (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.30, (0, 0, 0), 1)
                                cv2.imwrite(f"traj_predict/script/visualization/dataset/rgb_camera_top_groundtruth{val_index}_{batch_idx}.png", rgb_camera_top)  
                                ...
                                cv2.imwrite(f"traj_predict/script/visualization/dataset/rgb_camera_top_groundtruth_pred{val_index}_{batch_idx}.png", rgb_camera_top)  
